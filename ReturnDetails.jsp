...
<body>
    <div class="container">
        ...
        <!-- Search Form -->
        <form method="post">
            <input type="text" name="search_query" placeholder="Enter Student Card no. or Book ID">
        ...
        <%
            ...
            try {
                ...
                String query = "SELECT * FROM return_details";

                if (searchQuery != null && !searchQuery.trim().isEmpty()) {
                    query += " WHERE s_id LIKE ? OR b_id LIKE ?";
                ...
                if (!rs.isBeforeFirst()) { // If no records found
                    ...
                    out.println("<table>");
                    out.println("<tr><th>Student ID</th><th>Student Name</th><th>Card No</th><th>Book ID</th><th>Book Name</th><th>Issue Date</th><th>Return Date</th><th>Charges</th></tr>");

                    while (rs.next()) {
                        out.println("<tr>");
                        out.println("<td>" + rs.getInt("s_id") + "</td>");
                        out.println("<td>" + rs.getString("s_name") + "</td>");
                        out.println("<td>" + rs.getString("card_no") + "</td>");
                        out.println("<td>" + rs.getInt("b_id") + "</td>");
                        out.println("<td>" + rs.getString("b_name") + "</td>");
                        out.println("<td>" + rs.getString("issue_date") + "</td>");
                        out.println("<td>" + rs.getString("return_date") + "</td>");
                        out.println("<td>" + rs.getDouble("charges") + "</td>");
                        out.println("</tr>");
                    }
