        ...
        <%
            ...
            try {
                ...
                if (deleteId != null && !deleteId.isEmpty()) {
                    PreparedStatement deleteStmt = con.prepareStatement("DELETE FROM student WHERE s_rollno = ?");
                ...
                if (!rs.isBeforeFirst()) { // If no records found
                    ...
                    out.println("<table>");
                    out.println("<tr><th>Name</th><th>Class</th><th>Roll No</th><th>Gender</th><th>Phone No</th><th>Email</th><th>Action</th></tr>");
                    ...
                    while (rs.next()) {
                        ...
                        int rollNo = rs.getInt("s_rollno");

                        out.println("<tr>");
                        out.println("<td>" + sName + "</td>");
                        out.println("<td>" + rs.getString("s_class") + "</td>");
                        out.println("<td>" + rollNo + "</td>");
                        out.println("<td>" + rs.getString("s_gender") + "</td>");
                        out.println("<td>" + rs.getString("s_phno") + "</td>");
                        out.println("<td>" + rs.getString("s_email") + "</td>");
                        out.println("<td>");
                        out.println("<form method='post' onsubmit='return confirm(\"Are you sure you want to delete this student?\");'>");
                        out.println("<input type='hidden' name='delete_id' value='" + rollNo + "'>");
